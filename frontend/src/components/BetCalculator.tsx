...
import { Wallet, Coins, TrendingUp, AlertTriangle, Info } from 'lucide-react';
...
export default function BetCalculator({ runners, raceNo }: Props) {
  ...
  // Filter runners based on mode
  const filteredRunners = useMemo(() => {
    switch (mode) {
      case 'value':
        return runners.filter(r => r.is_value_bet);
      case 'all':
        return runners.filter(r => r.kelly_fraction > 0);
      case 'kelly':
        return runners.filter(r => r.kelly_fraction > 0.5); // Kelly > 0.5%
    }
  }, [runners, mode]);
  ...
  const stakes = useMemo(() => {
    return filteredRunners.map(r => {
      const kellyPct = r.kelly_fraction / 100; // kelly_fraction is already in %
      const stake = Math.round(bankroll * kellyPct);
      const potentialReturn = stake * r.win_odds;
      const expectedProfit = stake * (r.ev / 100); // ev is in %
      return {
        ...r,
        stake,
  ...
  if (runners.length === 0) {
    return (
      <div className="card text-[var(--text-muted)] text-center py-6">
  ...
  return (
    <div className="card space-y-4">
      ...
      <div className="flex gap-2">
        {[
          { key: 'value' as const, label: '+EV 價值投注', desc: '只計算通過 +EV 篩選的馬' },
          { key: 'kelly' as const, label: 'Kelly > 0.5%', desc: 'Kelly 份額較大的馬' },
          { key: 'all' as const, label: '所有正 Kelly', desc: '所有 Kelly > 0 的馬' },
        ].map(m => (
          <button
            key={m.key}
            ...
            <table className="data-table">
              ...
              <tbody>
                {stakes
                  ...
                  .map(s => (
                  <tr key={s.horse_no} className={s.is_value_bet ? 'signal-flash' : ''}>
                    <td className="font-mono">{s.horse_no}</td>
                    <td>
                      <span className="font-semibold">{s.horse_name}</span>
                      {s.horse_name_ch && <span className="text-[var(--text-muted)] ml-1 text-xs">({s.horse_name_ch})</span>}
                      {s.superhorse && <span className="ml-1 text-xs" style={{color: '#fbbf24'}}>👑</span>}
                      {s.ev_confidence === 'low' && <span className="ml-1 text-[9px] text-[var(--accent-red)]">⚠低信度</span>}
                    </td>
                    <td className="font-mono font-bold">{s.win_odds.toFixed(1)}</td>
                    <td className="font-mono font-bold" style={{ color: evColor(s.ev) }}>
                      {s.ev > 0 ? '+' : ''}{pct(s.ev)}
                    </td>
              ...
              <tfoot>
                <tr className="border-t-2 border-[var(--border)]">
                  <td colSpan={5} className="text-right text-sm font-bold text-[var(--text-secondary)]">合計</td>
